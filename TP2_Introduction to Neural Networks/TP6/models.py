import numpy as np
import TP6.losses as losses
import TP6.optimizers as optimizers
import TP6.metrics as metrics
import TP6.regularizers as reg
...
class Network:
	...
	def fit(self, x, y, test_data=None, epochs=100, loss=losses.MSE(), opt=optimizers.BGD(lr=None, bs=None), name='XOR', reg=reg.L2(lam=0)):
		...
		im_shape = x.shape[1:]
		x = np.reshape(x, (x.shape[0], np.prod(im_shape)))/255
		x_mean = np.mean(x[:, np.newaxis], axis=0)
		# x = (x - x_mean)
		if self.tdata:
			xtest = np.reshape(test_data[0], (test_data[0].shape[0], np.prod(im_shape))) / 255
