...
class Dense(WLayer):
	def __init__(self, units=None, activation=None, input_dim=None, factor=None):
	...
	def init_weights(self, units=1, x_dim=1):
		# self.W = np.random.normal(0, 0.1 * (np.shape(self.W)[0] + 1), (x_dim + 1, units))
		self.W = np.random.normal(0, 1, (x_dim + 1, units)) * self.factor
